...
export function isPipelineAssetReady(asset, lane) {
  return Boolean(asset && (asset.status === "OK" || (lane === "production_canary" && asset.status === "AI_PASS"))
    && asset.autoQc?.passed === true && asset.autoQc?.stale !== true);
}

export function pipelineTerminalState(lane) {
  return { status: "awaiting_user_review", terminal: true, h3Allowed: false, lane };
}
